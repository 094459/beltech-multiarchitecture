...
AWS_ECR_REPO=demo-beltech
AWS_CC_REPO=demo-beltech-repo
COMMIT_HASH="beltech"
...
if (aws codecommit get-repository --region $AWS_DEFAULT_REGION --repository-name $AWS_CC_REPO ) then
...
if (aws ecr describe-repositories --repository-names $AWS_ECR_REPO --region $AWS_DEFAULT_REGION ) then
	echo "Skipping the create repo as already exists"
else
	echo "Creating repos as it does not exists"
